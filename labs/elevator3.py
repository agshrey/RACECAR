...
def update():
    ...
    if cur_state == State.line:
        ...
        wait()
    elif cur_state == State.moveForward:
        print("got here 2 --> ", counter)
        if counter > 1:
            counter = 0
            cur_state = State.wall
        elif counter < 1:
    ...
    if cur_state != State.moveForward:
    ...
    print("total time:", counter2)
    counter2 += rc.get_delta_time()
    if counter2 > 71 and counter2 < 72:
        cur_state = State.wall
...
def go():
    ...
    print(left_distance, "<------>", right_distance)
    if left_distance > 110 and right_distance > 110 and len(markers) == 0:
        print("got here")
        cur_state = State.moveForward
